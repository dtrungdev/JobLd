...
import styles from "../styles/topCompany.module.scss";
...
function TopCompany() {
    const sliderCompany = useRef<object>(null);
    const slider = useRef<object>(null);
    ...
    const handleNextBtn = () => {
        ...
        if (
            ...
            -slider?.current?.offsetWidth +
                2 * (sliderCompany?.current?.offsetWidth + 16)
        ) {
    ...
    const style = {
        width: `${
            isLoading === false &&
            (sliderCompany?.current?.offsetWidth - 16 * 4) / 4
        }px`,
    };
        ...
        <section className={cl("wrapper")}>
            <div className={cl("content", "container")}>
                <div className={cl("top-company_header")}>
                    ...
                    <span className={cl("slider-btn")}>
                        <div className={cl("direct-page")}>
                            <i
                                className={cl("fa-solid fa-chevron-left")}
                                onClick={handlePreviousBtn}
                            ></i>
                        </div>
                        <div className={cl("direct-page")}>
                            <i
                                className={cl("fa-solid fa-chevron-right")}
                ...
                <div ref={sliderCompany} className={cl("top-company_box")}>
                    <div
                        ref={slider}
                        className={cl("top-company_slider")}
                        style={styleSlider}
                    >
                        {isLoading === false ? (
                            <>
